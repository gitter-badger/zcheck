...
object ZCheckBuild extends Build{
  ...
  ) //uncomment for saucelabs stuff: ++ concatAllSjsDependencies ++ autoOpt
  ...
  lazy val zcheckJs          = zcheckModule.jsProject(zcheckSharedJs).dependsOn(zcoreJs).dependsOn(zcheckSharedJs % "compile;test->test")
